...
```mermaid
flowchart TD
  Q["Negating a modal — which meaning?"] -->|"not allowed"| A["mag niet — Je mag niet roken"]
  Q -->|"not necessary"| B["hoeft niet te — Je hoeft niet te komen"]
  Q -->|"common trap"| C["not moet niet — it fits neither"]
```
