...
| `uit` origin | out of | *uit de tas, uit Amsterdam, uit het raam* |
| `met` peer | with | Ik schrijf *met* een pen. |
| `voor` target | for | Dit is *voor* jou. |
| `per` init | by (means) | Ik kom *per* trein. |
...
| Dutch | English | Example |
|-------|---------|---------|
| `in` period | in (months, years, seasons) | `In* januari sneeuwt het. |
| `op` day/date | on (day/date) | `Op* maandag werk ik thuis. |
...
| Dutch | English | Example |
|-------|---------|---------|
| `in` area, stad, land, huis | inside a bounded space | in het huis, in Amsterdam |
...
| `voorbij` | over, all over, beyond, by / by someone's place / just happens | *Hij woont net voorbij het kruispunt* |
...
| `dadelijk` | in a moment | Ik kom **dadelijk** bij je. (I'll come to you in a moment.) |
...
Once you can build single-tense sentences, the next step is sequencing them.
...
| `vorig` | last / previous | In de `vorige` jaar  |
| `volgende` | next | Op `volgende` halte |
...
| Dutch | English | Example |
|-------|---------|---------|
| `tot` / `totdat` | until | We blijven **tot** zeven uur. Wacht *totdat* ik kom. |
...
| `ginds` | over there | Het station ligt **ginds**, achter de kerk. |
